...
$md += ''
$md += "- AGTF30 commit: `$($record.AGTF30_commit)`"
$md += "- T-MATS commit: `$($record.TMATS_commit)`"
...
else { foreach ($x in ($tmatsRefs | Sort-Object)) { $md += "- `$x`" } }
$md += ''
$md += '## Cantera markers'
if ($canteraHits.Count -eq 0) { $md += '- None found in the exact AGTF30 execution-source files scanned.' }
else { foreach ($x in ($canteraHits | Sort-Object -Unique)) { $md += "- `$x`" } }
